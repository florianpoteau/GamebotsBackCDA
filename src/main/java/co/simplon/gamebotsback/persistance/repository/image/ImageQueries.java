...
public final class ImageQueries {
   ...
   */
  public static final String FIND_IMAGE_BY_USERID =
      "SELECT i FROM User u JOIN u.image i JOIN i.typeImage ti WHERE ti.name ="
          + " 'avatar' AND u.id = :userId";
  /**
   * Query to find all images associated with a game by game ID.
   */
  public static final String FIND_ALL_IMAGES_BY_GAMEID =
      "SELECT i FROM Image i JOIN i.typeImage ti JOIN i.game g WHERE ti.name ="
